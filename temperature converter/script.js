...
// Realtime Update
$('#inputD').on('input', () => convertInputDegree());
$('#InputDegreeType').change(() => convertInputDegree());
$('#ConversionType').change(() => convertInputDegree());
// Main function which chechks the input and output degree type and then launch the function
function convertInputDegree() {
    let inputDegree = parseInt($('#inputD').val());
    let selectInputDegreeType = $('#InputDegreeType').val();
    ...
    $('#convertU').text(conversionType)
    // To update the Degree Value
    $('#convertD').text(resultValue.toFixed(2));
}
